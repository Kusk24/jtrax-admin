...
import { STUDENTS_SEED, type Student } from "@/lib/jtrax/data";
import { buildAttendanceRows, buildPracticeStrip, buildStudentPayments } from "@/lib/jtrax/derive";
import { Icon } from "@/lib/jtrax/icons";
import { classDotColor, COLORS, FONT, initialsOf, statusChipColors } from "@/lib/jtrax/theme";
import {
  EmptyRow,
  fieldStyle,
  FilterBar,
...
const STATUS_OPTIONS = ["All Statuses", "Normal", "Low Credit", "Expiring", "Expired", "Inactive"];
...
function StudentDetail({ student, onBack }: { student: Student; onBack: () => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      >
        <Icon name="chevronLeft" size={16} color={COLORS.textSecondary} /> Back to Students
      </button>
      ...
      <Card style={{ display: "flex", alignItems: "center", gap: 14, flexWrap: "wrap" }}>
        ...
        <div style={{ flex: "1 1 220px", minWidth: 0 }}>
          ...
          <div style={{ marginTop: 3, fontFamily: FONT, fontSize: 13, color: COLORS.textSecondary }}>
            {student.age} yrs · {student.level} · {student.id}
          </div>
        </div>
          ...
          <Card style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <InfoGrid
              rows={[
                ...
                { label: "Membership", value: student.membershipType },
                { label: "Joined", value: student.joinedDate },
                { label: "Credits expire", value: student.expires },
                { label: "LINE ID", value: student.studentLineId },
              ]}
            />
          </Card>
          <Card style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <InfoGrid
              rows={[
                ...
                { label: "Relation", value: student.parentRelation },
                { label: "Phone", value: student.parentPhone },
                { label: "Email", value: student.parentEmail },
                { label: "LINE ID", value: student.parentLineId },
              ]}
            />
          </Card>
        </div>
      ...
      {tab === "Attendance" && (
        <Card style={{ padding: 0, overflow: "hidden" }}>
          <div style={{ padding: "16px 16px 0" }}>
            <SectionTitle>
              Attendance — {presentCount} of {attendance.length} present
            </SectionTitle>
          </div>
          <div style={{ marginTop: 12 }}>
            <Table columns={["Date", "Class", "Status"]} template="140px 1fr 110px" minWidth={480}>
              {attendance.map((row) => {
      ...
      {tab === "Payments" && (
        <Card style={{ padding: 0, overflow: "hidden" }}>
          <Table columns={["Class", "Credits", "Amount", "Date", "Method"]} template="1.4fr 90px 120px 130px 130px" minWidth={640}>
            {payments.map((p, i) => (
...
function AddStudentWizard({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, maxWidth: 820 }}>
      ...
      <PageHeader title="Register Student" sub="Upload a registration form or enter the details manually." />

      {stage === "choice" && (
        <div className="jt-duo">
          {(
            [
              { key: "upload", icon: "fileText" as const, title: "Upload Registration Form", desc: "We'll read the details off the form for you." },
              { key: "manual", icon: "edit" as const, title: "Manual Entry", desc: "Type the student and guardian details in yourself." },
            ]
          ).map((choice) => (
            <button
              key={choice.key}
        ...
        <>
          {docName && (
            <Card style={{ display: "flex", alignItems: "center", gap: 9, padding: "11px 14px" }}>
              ...
              <span style={{ fontFamily: FONT, fontSize: 12.5, color: COLORS.textSecondary }}>
                Pre-filled from <strong style={{ color: COLORS.text }}>{docName}</strong> — please check every field.
              </span>
            </Card>
          )}
          ...
          <Card style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div className="jt-duo">
              <div>
                <label style={labelStyle} htmlFor="w-name">Full Name</label>
                <input id="w-name" value={draft.name} onChange={(e) => set("name", e.target.value)} style={fieldStyle} />
              </div>
              <div>
                <label style={labelStyle} htmlFor="w-class">Class</label>
              ...
              <div>
                <label style={labelStyle} htmlFor="w-school">Current School</label>
                <input id="w-school" value={draft.school} onChange={(e) => set("school", e.target.value)} style={fieldStyle} />
              </div>
              <div>
                <label style={labelStyle} htmlFor="w-fide">FIDE ID (optional)</label>
          ...
          <Card style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div className="jt-duo">
              <div>
                <label style={labelStyle} htmlFor="w-pname">Name</label>
                <input id="w-pname" value={draft.parentName} onChange={(e) => set("parentName", e.target.value)} style={fieldStyle} />
              </div>
              <div>
                <label style={labelStyle} htmlFor="w-prel">Relation</label>
              ...
              <div>
                <label style={labelStyle} htmlFor="w-pphone">Phone Number</label>
                <input id="w-pphone" value={draft.parentPhone} onChange={(e) => set("parentPhone", e.target.value)} style={fieldStyle} />
              </div>
              <div>
                <label style={labelStyle} htmlFor="w-pmail">Email</label>
...
export function StudentsPage({ presetSearch, startWizard }: { presetSearch?: string; startWizard?: string }) {
  const [students, setStudents] = useState<Student[]>(STUDENTS_SEED);
  const [view, setView] = useState<View>(startWizard !== undefined ? { kind: "wizard" } : { kind: "list" });
  ...
  const branchOptions = useMemo(
    () => ["All Branches", ...Array.from(new Set(STUDENTS_SEED.map((s) => s.branch)))],
    [],
  );
  ...
  const filtered = useMemo(() => {
    ...
    return students.filter((s) => {
      ...
      if (branch !== "All Branches" && s.branch !== branch) return false;
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <PageHeader
        title="Students"
        sub="Enrolled students and profiles"
        action={
          <button
            type="button"
            className="jt-btn-primary"
      ...
      <FilterBar>
        ...
        />
        <SelectFilter value={branch} onChange={(v) => { setBranch(v); setPage(0); }} options={branchOptions} label="Filter by branch" />
        <SelectFilter value={status} onChange={(v) => { setStatus(v); setPage(0); }} options={STATUS_OPTIONS} label="Filter by status" />
      </FilterBar>

      <Card style={{ padding: 0, overflow: "hidden" }}>
        <Table columns={["Student", "Class", "Credit", "Status", "Action"]} template={TEMPLATE} minWidth={760}>
          {pageRows.length === 0 && <EmptyRow>No students match these filters.</EmptyRow>}
          {pageRows.map((s) => {
